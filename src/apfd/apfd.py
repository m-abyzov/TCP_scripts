...
def calculate_APFD(tests_order, test_info, bugs_count, print_APFD=True):
    apfd_numerator = 0
...
def estimate_single_experiment(TEST_ORDER_PATH, BUGS_INFO_PATH, print_orders=True):
    ...
    return calculate_APFD(calculated_order, tests_revealings, bugs_count, print_orders)


def get_bugs_info_path_by_project_id(PROJECT_ID):
    return f"D4J/{PROJECT_ID}/{PROJECT_ID}_bugs_revealing_info.csv"
